# -*- coding: utf-8 -*-
"""
Created on Tue Apr 24 17:30:01 2018
...
pre_in = pre_x[0:5, :]
for i in range(1, (pre_x.shape[0] - 4)):
    pre_in = concatenate((pre_in, pre_x[i:i+5, :]), axis = 0)
print(pre_in.shape)
pre_out = pre_y[4:]
print(pre_out.shape)
pre_in = pre_in.reshape(((int(int(pre_in.shape[0]) / 5)), 5, pre_in.shape[1]))
print(pre_in.shape, pre_out.shape)

# predict
model = load_model("D:/python_workspace/SF/work2/data/lstm_model_step_5.h5")
...
# save result
fo = "D:/python_workspace/SF/work2/data/VIB8_result.txt"

time = pre_dataset.index.tolist()
time_test = time[4:7000]
