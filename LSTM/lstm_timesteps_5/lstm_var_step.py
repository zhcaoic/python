# -*- coding: utf-8 -*-
"""
Created on Tue Apr 24 14:46:11 2018
...
train_in = train_x[0:5, :]
test_in = test_x[0:5, :]
for i in range(1, (train_x.shape[0] - 4)):
    train_in = concatenate((train_in, train_x[i:i+5, :]), axis = 0)
for j in range(1, (test_x.shape[0] - 4)):
    test_in = concatenate((test_in, test_x[j:j+5, :]), axis = 0)
print(train_in.shape, test_in.shape)
train_out = train_y[4:]
test_out = test_y[4:]
print(train_out.shape, test_out.shape)
train_in = train_in.reshape(((int(int(train_in.shape[0]) / 5)), 5, train_in.shape[1]))
test_in = test_in.reshape(((int(int(test_in.shape[0]) / 5)), 5, test_in.shape[1]))
...
pyplot.show()
# save model
model.save("D:/python_workspace/SF/work2/data/lstm_model_step_5.h5")
